...
def staple_to_front(s: str, c: str) -> str:
    ...
    """
    # by using the plus sign we concatenate(add together) the strings c and s 
    # this creates a combined string
    result = c + s 
    return result
...
def staple_to_end(s: str, c: str) -> str:
    ...
    """
    # by switching the order of the strings s and c it allows the output to be reversed
    result = s + c  
    return result
...
def shred_first_character(s: str) -> str:
    ...
    """
    # the first part of the slice tells us that the character at the index of 1 will be shredded
    # the rest of the word will be printed 
    return s[1:] 
